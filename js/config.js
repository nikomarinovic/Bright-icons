...
const SITE_CONFIG = {
  ...
  pages: {
    ...
    underDevelopment: '/public/dev/under-development.html',
  },
};
/* ─────────────────────────────────────────
...
(function(){
  ...
  if(cfg.mobileRedirect&&!isWallPage()){
    ...
    if(!bypassed){
      const isMobile=window.innerWidth<cfg.mobileBreakpoint||/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
      if(isMobile){window.location.replace(cfg.pages.mobileWall);}
    }
  }
